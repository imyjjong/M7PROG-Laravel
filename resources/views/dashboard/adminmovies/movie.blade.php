...
<body class="min-h-screen bg-background text-color flex flex-col justify-start">
    ...
    <main class="absolute -bottom-24 w-full h-fit px-52 flex gap-12 justify-center items-start">
    <aside class="flex justify-center items-start w-fit h-full mb-10">
    ...
    <section class="relative w-4/6 h-fit flex flex-col justify-center items-center">
        <div class="absolute top-0 w-full flex flex-col justify-center items-start gap-6">
            ...
            <div class="bg-lighten transition-transform rounded-sm flex flex-col w-72 h-fit justify-center items-end gap-4 p-4 pt-1">
                <ul class="flex justify-center items-center gap-10 border-b w-full py-3">
                    <li class="flex flex-col justify-center items-center">
                        <i class="{{ $movie->watched }} text-4xl hover:text-flashyButton"></i>
                        <p>@if(old('watched', $movie->watched === 'fas fa-eye')) Watched @else Watchlist @endif</p>
                    </li>
                    <li class="flex flex-col justify-center items-center">
                        <i class="{{ $movie->like }} text-4xl hover:text-flashyButton"></i>
                        <p>@if(old('watched', $movie->like === 'fas fa-heart')) Liked @else Like @endif</p>
                    </li>
                    <li class="flex flex-col justify-center items-center">   
                        <a href="{{ route('adminmovies.edit', ['adminmovie' => $movie->id]) }}"><i class="fas fa-pen text-4xl hover:text-flashyButton"></i></a>
                        <p>Edit</p>
                    </li>
                </ul>
                <article class="cast-members w-full h-full shadow-lg flex justify-start items-center gap-2 flex-wrap border-b pb-3">
                    @php
                ...
                <article class="genres w-full h-full shadow-lg flex justify-start items-center gap-2 flex-wrap">
                    @php
                        $genres = explode(', ', $movie->genre);
                    @endphp
                    @foreach($genres as $genre)
                        <form action="{{ route('genres.genres') }}" method="POST">
                            @method('POST')
                            @csrf
                            <input type="hidden" name="genre" value="{{ $genre }}">
                            <button type="submit" class="bg-cast text-castText w-max py-0.5 px-2 rounded-sm shadow flex justify-center items-center hover:cursor-pointer hover:brightness-110">{{ $genre }}</button>
                        </form>
                    @endforeach
                </article>
            </div>
